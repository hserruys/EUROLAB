...
Labels_cols= (("Education"\""\""\"Age"\""\""\"Child"\""\"Total"),("Low level"\"Middle level"\"High level"\"20-30"\"31-40"\"41-on"\"Yes"\"No"\""))
...
for (i=1;i<=3;i++){  //all men women
	...
	sections_mat[i,1]=&(*tmat[1,i]\*tmat[2,i]\*tmat[3,i])
...
foreach table in "all" "men" "women" {
	...
	foreach type in "all" "couples" "singles" {
		...
		if "`type'" == "singles" {
			local condition "if inlist(flex_hh,2,3)"
		}
		...
		di in r "ch_`i'`j'"
		mat ch_`i'`j'=(r(Stat1)\r(Stat2)\r(StatTotal))
		mat list ch_`i'`j'
	}	
}	

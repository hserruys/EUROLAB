...
*Labels_cols= (("Education"\""\""\"Age"\""\""\"Child"\""\"Minimum Wage"\""\"Migrant"\""\"Total"),("Low level"\"Middle level"\"High level"\"20-30"\"31-40"\"41-on"\"Yes"\"No"\"Yes"\"No"\"Yes"\"No"\""))

Labels_cols= (("Education"\""\""\"Age"\""\""\"Child"\""\"Minimum Wage"\""\"Migrant"\""\"Income_Quintile"\""\""\""\""\"Total"),("Low level"\"Middle level"\"High level"\"20-30"\"31-40"\"41-on"\"Yes"\"No"\"Yes"\"No"\"Yes"\"No"\"1"\"2"\"3"\"4"\"5\""))
...
//Set number format
b.set_number_format((2,60),(4,12),"0.000")
...
foreach table in "all" "men" "women" {
	...
	foreach type in "all" "couples" "singles" {
		...
		//di in r "`type'"
		if "`type'" == "all" {
			local condition "if inlist(flex_hh,1,2,3)"
		}
		...
		di in r "ch_`i'`j'"
		mat ch_`i'`j'=(r(Stat1)\r(Stat2)) //REMOVE TOTAL YOU ONLY WANT IT IN THE LAST VARIABLE MW
		mat list ch_`i'`j'
		
		tabstat ela_tot_own ela_ext_own ela_int_own [aw = dwt] `condition' & `cond_tab',stat(mean) by(MW_earner) save 
		di in r "MW_`i'`j'"
		mat MW_`i'`j'=(r(Stat1)\r(Stat2)\r(StatTotal))
		mat list MW_`i'`j'
		
		tabstat ela_tot_own ela_ext_own ela_int_own [aw = dwt] `condition' & `cond_tab',stat(mean) by(MW_earner) save 
		di in r "Mig_`i'`j'"
		mat Mig_`i'`j'=(r(Stat1)\r(Stat2)\r(StatTotal))
		mat list Mig_`i'`j'
		
		tabstat ela_tot_own ela_ext_own ela_int_own [aw = dwt] `condition' & `cond_tab',stat(mean) by(MW_earner) save 
		di in r "pct_`i'`j'"
		mat pct_`i'`j'=(r(Stat1)\r(Stat2)\r(StatTotal))
		mat list pct_`i'`j'
	}	
}	

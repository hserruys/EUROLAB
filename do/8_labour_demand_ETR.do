...
quiet do 8_programs_equilibrium_ETR.do
...
foreach ref in $new_reforms{
	di in r "`ref'"
	use "${path_LSscen}/4_pred/${country}_${year}_${choices}ch_pred_LD.dta", clear 
	gen cons = 0
	...
	if `baseline_ls' {
		...
		amoeba TAXFUNCTION btaxk yout btax . 
		
		mat tax = 1/(1+exp(btax[1,1]))
